 ...
 * 3，一个用户可以属于多个用户组(Sower_auth_group_access表 定义了用户所属用户组)。我们需要设置每个用户组拥有哪些规则(think_auth_group 定义了用户组权限)
 * 4，支持规则表达式。
 *      在Sower_auth_rule 表中定义一条规则，condition字段就可以定义规则表达式。 如定义{score}>5  and {score}<100
...
class Auth
{
    ...
    public function getRuleList($uid)
    {
		...
		$rulelist = []; 
        if (!in_array('*', $ids)) {
        $this->rules = Db::name($this->config['auth_rule'])->where('id', 'in',$ids )->field('id,pid,condition,icon,name,title,ismenu')->select();
        }else if (in_array('*', $ids)) {
		$rulelist[] = "*";
		$this->rules = Db::name($this->config['auth_rule'])->field('id,pid,condition,icon,name,title,ismenu')->select();
