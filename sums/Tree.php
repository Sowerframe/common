...
class Tree
{
    ...
    public function getView($myid, $itemtpl, $selectedids = '', $disabledids = '', $wraptag = 'ul', $wrapattr = '', $deeplevel = 0)
    {
        $str = '';
        ...
        if ($childs) {
            foreach ($childs as $value) {
                ...
                $childdata = $this->getView($id, $itemtpl, $selectedids, $disabledids, $wraptag, $wrapattr, $deeplevel + 1);
                $childlist = $childdata ? "<dl class='layui-nav-child'> <dd data-name='list' class='@class'>" . $childdata . "</dd></dl>" : "";
                $childlist = strtr($childlist, array('@class' => $childdata ? 'last' : ''));
                $var=url($value['@name']);
				$value = array(
                    '@childlist' => $childlist,
                    '@url'       => $childdata || !isset($value['@url']) ? "href='javascript:;'" :" href='javascript:;' lay-href='$var'",
                    '@addtabs'   => $childdata || !isset($value['@url']) ? "" : (stripos($value['@url'], "?") !== false ? "&" : "?") . "action=back",
                    '@caret'     => ($childdata && (!isset($value['@badge']) || !$value['@badge']) ? '<i class="ss ss-angle-left"></i>' : ''),
