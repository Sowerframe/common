...
class Auth extends \sums\Auth
{
    ...
    public function isLogin()
    {
        ...
        if (Config::get('sums.login_unique')) {
    ...
    public function getBreadCrumb($path = '')
    {
        ...
        if ($path_rule_id) {
            ...
            foreach ($this->breadcrumb as $k => &$v) {
                $v['url'] = url($v['name']);
                $v['title'] = SoLan($v['title']);
            }
        }
        return $this->breadcrumb;
    }
    public function navone($params = [])
    {
        $module = request()->app();
        // 读取管理员当前拥有的权限节点
        ...
        $menu = Tree::instance()->getView(0, '<li data-name="@py" class="layui-nav-item @class">
              <a   @url  lay-tips="@title" lay-direction="@id">
                <i class="@icon"></i>
                <cite>@title</cite>
              </a>
             @childlist
            </li>',$select_id, '', 'ul', 'class="layui-nav-more"');
        return [$menu];
    }	
    /**
     * 获取左侧和顶部菜单栏
    ...
    {
        return $this->_error ? SoLan($this->_error) : '';
    }
}
